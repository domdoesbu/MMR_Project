...
namespace MeshDecimation
{
    ...
    void MeshDecimator::Initialize(UnstructuredGrid3D& grid)
    {
        decPoints.resize(grid.numPoints());
        decTriangles.resize(grid.numCells());
		  
        for(int i=0;i<grid.numPoints();++i)
        {
		     float p[3];
		     grid.getPoint(i,p);
		     decPoints[i] = Vec3<Float>(p[0],p[1],p[2]);
        }
        for(int i=0;i<grid.numCells();++i)
        {
	             int c[3];
	             grid.getCell(i,c);
	             decTriangles[i] = Vec3<int>(c[0],c[1],c[2]);
        } 			     	          
	        
        Initialize(decPoints.size(), decTriangles.size(), &decPoints[0], &decTriangles[0]);
    }
    ...
    void MeshDecimator::GetDecimatedOutput(UnstructuredGrid3D*& grid)
    {
        vector< Vec3<Float> > decPoints(GetNVertices());
        vector< Vec3<int> > decTriangles(GetNTriangles());
        GetMeshData(&decPoints[0], &decTriangles[0]);

        grid = new UnstructuredGrid3D(GetNVertices(),GetNTriangles());
        for(int i=0;i<GetNVertices();++i)
            grid->setPoint(i,&decPoints[i][0]);
        for(int i=0;i<GetNTriangles();++i)
            grid->setCell(i,&decTriangles[i][0]);    
    ...
    double MeshDecimator::ComputeEdgeCost(int v1, int v2, Vec3<Float> & newPos) const
    {
        ...
        else
        {
            ...
            Vec3<Float> mv2 = { m_points[v2].X() * w, m_points[v2].Y() * w, m_points[v2].Z() * w };
            Vec3<Float> mv1 = { m_points[v1].X() * w, m_points[v1].Y() * w, m_points[v1].Z() * w };
            newPos = mv1 + mv2;
