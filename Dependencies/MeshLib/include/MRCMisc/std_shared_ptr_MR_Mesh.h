...
/// Wraps a pointer to a single shared reference-counted heap-allocated `MR::Mesh`.
...
/// The array must be destroyed using `MR_std_shared_ptr_MR_Mesh_DestroyArray()`.
/// Use `MR_std_shared_ptr_MR_Mesh_OffsetMutablePtr()` and `MR_std_shared_ptr_MR_Mesh_OffsetPtr()` to access the array elements.
...
/// This being zero usually conincides with `MR_std_shared_ptr_MR_Mesh_Get()` returning null, but is ultimately orthogonal.
