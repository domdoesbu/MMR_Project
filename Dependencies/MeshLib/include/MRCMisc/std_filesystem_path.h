...
/// The array must be destroyed using `MR_std_filesystem_path_DestroyArray()`.
/// Use `MR_std_filesystem_path_OffsetMutablePtr()` and `MR_std_filesystem_path_OffsetPtr()` to access the array elements.
MRC_API MR_std_filesystem_path *MR_std_filesystem_path_DefaultConstructArray(size_t num_elems);

/// Constructs a copy of another instance. The source remains alive.
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_filesystem_path_Destroy()` to free it when you're done using it.
MRC_API MR_std_filesystem_path *MR_std_filesystem_path_ConstructFromAnother(MR_PassBy other_pass_by, MR_std_filesystem_path *other);

/// Constructs a new instance.
/// Parameter `other` is a UTF-8 encoded filesystem path.
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_filesystem_path_Destroy()` to free it when you're done using it.
MRC_API MR_std_filesystem_path *MR_std_filesystem_path_ConstructFrom(const char *other, const char *other_end);
...
MRC_API const MR_std_filesystem_path *MR_std_filesystem_path_OffsetPtr(const MR_std_filesystem_path *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_filesystem_path *MR_std_filesystem_path_OffsetMutablePtr(MR_std_filesystem_path *ptr, ptrdiff_t i);
