...
/// Wraps a pointer to a single shared reference-counted heap-allocated `MR::ObjectLinesHolder`.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_shared_ptr_MR_ObjectLinesHolder_Destroy()` to free it when you're done using it.
...
/// The array must be destroyed using `MR_std_shared_ptr_MR_ObjectLinesHolder_DestroyArray()`.
/// Use `MR_std_shared_ptr_MR_ObjectLinesHolder_OffsetMutablePtr()` and `MR_std_shared_ptr_MR_ObjectLinesHolder_OffsetPtr()` to access the array elements.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_shared_ptr_MR_ObjectLinesHolder_Destroy()` to free it when you're done using it.
MRC_API MR_std_shared_ptr_MR_ObjectLinesHolder *MR_std_shared_ptr_MR_ObjectLinesHolder_ConstructFromAnother(MR_PassBy other_pass_by, MR_std_shared_ptr_MR_ObjectLinesHolder *other);
...
MRC_API const MR_std_shared_ptr_MR_ObjectLinesHolder *MR_std_shared_ptr_MR_ObjectLinesHolder_OffsetPtr(const MR_std_shared_ptr_MR_ObjectLinesHolder *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_shared_ptr_MR_ObjectLinesHolder *MR_std_shared_ptr_MR_ObjectLinesHolder_OffsetMutablePtr(MR_std_shared_ptr_MR_ObjectLinesHolder *ptr, ptrdiff_t i);
...
/// This being zero usually conincides with `MR_std_shared_ptr_MR_ObjectLinesHolder_Get()` returning null, but is ultimately orthogonal.
/// Note that in multithreaded environments, the only safe way to use this number is comparing it with zero. Positive values might change by the time you get to use them.
/// Parameter `_this` can not be null. It is a single object.
MRC_API int MR_std_shared_ptr_MR_ObjectLinesHolder_UseCount(const MR_std_shared_ptr_MR_ObjectLinesHolder *_this);
...
/// Parameter `ptr` takes ownership of the passed pointer (if not null), and will later call `MR_ObjectLinesHolder_Destroy()` on it automatically.
MRC_API void MR_std_shared_ptr_MR_ObjectLinesHolder_Construct(MR_ObjectLinesHolder *ptr);
