...
/// The array must be destroyed using `MR_std_pair_MR_VertId_MR_VertId_DestroyArray()`.
/// Use `MR_std_pair_MR_VertId_MR_VertId_OffsetMutablePtr()` and `MR_std_pair_MR_VertId_MR_VertId_OffsetPtr()` to access the array elements.
...
/// Parameter `other` can not be null. It is a single object.
MRC_API void MR_std_pair_MR_VertId_MR_VertId_AssignFromAnother(MR_std_pair_MR_VertId_MR_VertId *_this, const MR_std_pair_MR_VertId_MR_VertId *other);
...
MRC_API const MR_std_pair_MR_VertId_MR_VertId *MR_std_pair_MR_VertId_MR_VertId_OffsetPtr(const MR_std_pair_MR_VertId_MR_VertId *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_pair_MR_VertId_MR_VertId *MR_std_pair_MR_VertId_MR_VertId_OffsetMutablePtr(MR_std_pair_MR_VertId_MR_VertId *ptr, ptrdiff_t i);

/// Constructs the pair elementwise.
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_pair_MR_VertId_MR_VertId_Destroy()` to free it when you're done using it.
MRC_API MR_std_pair_MR_VertId_MR_VertId *MR_std_pair_MR_VertId_MR_VertId_Construct(MR_VertId first, MR_VertId second);
