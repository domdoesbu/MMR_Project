...
/// Wraps a pointer to a single shared reference-counted heap-allocated `const MR::SceneRootObject`.
...
/// The array must be destroyed using `MR_std_shared_ptr_const_MR_SceneRootObject_DestroyArray()`.
/// Use `MR_std_shared_ptr_const_MR_SceneRootObject_OffsetMutablePtr()` and `MR_std_shared_ptr_const_MR_SceneRootObject_OffsetPtr()` to access the array elements.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_shared_ptr_const_MR_SceneRootObject_Destroy()` to free it when you're done using it.
MRC_API MR_std_shared_ptr_const_MR_SceneRootObject *MR_std_shared_ptr_const_MR_SceneRootObject_ConstructFromAnother(MR_PassBy other_pass_by, MR_std_shared_ptr_const_MR_SceneRootObject *other);
...
MRC_API const MR_std_shared_ptr_const_MR_SceneRootObject *MR_std_shared_ptr_const_MR_SceneRootObject_OffsetPtr(const MR_std_shared_ptr_const_MR_SceneRootObject *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_shared_ptr_const_MR_SceneRootObject *MR_std_shared_ptr_const_MR_SceneRootObject_OffsetMutablePtr(MR_std_shared_ptr_const_MR_SceneRootObject *ptr, ptrdiff_t i);
...
/// This being zero usually conincides with `MR_std_shared_ptr_const_MR_SceneRootObject_Get()` returning null, but is ultimately orthogonal.
