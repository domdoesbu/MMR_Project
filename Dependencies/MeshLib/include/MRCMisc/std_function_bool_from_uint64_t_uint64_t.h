...
/// Stores a functor of type: `bool(MR_uint64_t, MR_uint64_t)`. Possibly stateful.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_function_bool_from_uint64_t_uint64_t_Destroy()` to free it when you're done using it.
...
/// The array must be destroyed using `MR_std_function_bool_from_uint64_t_uint64_t_DestroyArray()`.
/// Use `MR_std_function_bool_from_uint64_t_uint64_t_OffsetMutablePtr()` and `MR_std_function_bool_from_uint64_t_uint64_t_OffsetPtr()` to access the array elements.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_function_bool_from_uint64_t_uint64_t_Destroy()` to free it when you're done using it.
MRC_API MR_std_function_bool_from_uint64_t_uint64_t *MR_std_function_bool_from_uint64_t_uint64_t_ConstructFromAnother(MR_PassBy other_pass_by, MR_std_function_bool_from_uint64_t_uint64_t *other);
...
MRC_API const MR_std_function_bool_from_uint64_t_uint64_t *MR_std_function_bool_from_uint64_t_uint64_t_OffsetPtr(const MR_std_function_bool_from_uint64_t_uint64_t *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_function_bool_from_uint64_t_uint64_t *MR_std_function_bool_from_uint64_t_uint64_t_OffsetMutablePtr(MR_std_function_bool_from_uint64_t_uint64_t *ptr, ptrdiff_t i);
