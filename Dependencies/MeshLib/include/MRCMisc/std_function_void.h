...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_function_void_Destroy()` to free it when you're done using it.
MRC_API MR_std_function_void *MR_std_function_void_DefaultConstruct(void);

/// Constructs an array of empty (default-constructed) instances, of the specified size. Will never return null.
/// The array must be destroyed using `MR_std_function_void_DestroyArray()`.
/// Use `MR_std_function_void_OffsetMutablePtr()` and `MR_std_function_void_OffsetPtr()` to access the array elements.
