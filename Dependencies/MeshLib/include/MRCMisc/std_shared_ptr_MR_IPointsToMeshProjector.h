...
typedef struct MR_IPointsToMeshProjector MR_IPointsToMeshProjector; // Defined in `#include <MRCMesh/MRPointsToMeshProjector.h>`.


/// Wraps a pointer to a single shared reference-counted heap-allocated `MR::IPointsToMeshProjector`.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_shared_ptr_MR_IPointsToMeshProjector_Destroy()` to free it when you're done using it.
...
/// The array must be destroyed using `MR_std_shared_ptr_MR_IPointsToMeshProjector_DestroyArray()`.
/// Use `MR_std_shared_ptr_MR_IPointsToMeshProjector_OffsetMutablePtr()` and `MR_std_shared_ptr_MR_IPointsToMeshProjector_OffsetPtr()` to access the array elements.
...
/// Never returns null. Returns an instance allocated on the heap! Must call `MR_std_shared_ptr_MR_IPointsToMeshProjector_Destroy()` to free it when you're done using it.
MRC_API MR_std_shared_ptr_MR_IPointsToMeshProjector *MR_std_shared_ptr_MR_IPointsToMeshProjector_ConstructFromAnother(MR_PassBy other_pass_by, MR_std_shared_ptr_MR_IPointsToMeshProjector *other);
...
MRC_API const MR_std_shared_ptr_MR_IPointsToMeshProjector *MR_std_shared_ptr_MR_IPointsToMeshProjector_OffsetPtr(const MR_std_shared_ptr_MR_IPointsToMeshProjector *ptr, ptrdiff_t i);

/// Offsets a pointer to an array element by `i` positions (not bytes). Use only if you're certain that the pointer points to an array element.
MRC_API MR_std_shared_ptr_MR_IPointsToMeshProjector *MR_std_shared_ptr_MR_IPointsToMeshProjector_OffsetMutablePtr(MR_std_shared_ptr_MR_IPointsToMeshProjector *ptr, ptrdiff_t i);
...
/// This being zero usually conincides with `MR_std_shared_ptr_MR_IPointsToMeshProjector_Get()` returning null, but is ultimately orthogonal.
...
/// Parameter `ptr` takes ownership of the passed pointer (if not null), and will later call `MR_IPointsToMeshProjector_Destroy()` on it automatically.
