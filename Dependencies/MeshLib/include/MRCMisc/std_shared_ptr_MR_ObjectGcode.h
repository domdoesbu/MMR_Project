...
/// The array must be destroyed using `MR_std_shared_ptr_MR_ObjectGcode_DestroyArray()`.
/// Use `MR_std_shared_ptr_MR_ObjectGcode_OffsetMutablePtr()` and `MR_std_shared_ptr_MR_ObjectGcode_OffsetPtr()` to access the array elements.
...
/// Destroys a heap-allocated instance of `MR_std_shared_ptr_MR_ObjectGcode`. Does nothing if the pointer is null.
MRC_API void MR_std_shared_ptr_MR_ObjectGcode_Destroy(const MR_std_shared_ptr_MR_ObjectGcode *_this);

/// Destroys a heap-allocated array of `MR_std_shared_ptr_MR_ObjectGcode`. Does nothing if the pointer is null.
...
/// This being zero usually conincides with `MR_std_shared_ptr_MR_ObjectGcode_Get()` returning null, but is ultimately orthogonal.
...
/// Parameter `ptr` takes ownership of the passed pointer (if not null), and will later call `MR_ObjectGcode_Destroy()` on it automatically.
