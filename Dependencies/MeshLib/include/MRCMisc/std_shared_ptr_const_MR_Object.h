...
/// Wraps a pointer to a single shared reference-counted heap-allocated `const MR::Object`.
...
/// The array must be destroyed using `MR_std_shared_ptr_const_MR_Object_DestroyArray()`.
/// Use `MR_std_shared_ptr_const_MR_Object_OffsetMutablePtr()` and `MR_std_shared_ptr_const_MR_Object_OffsetPtr()` to access the array elements.
...
/// This being zero usually conincides with `MR_std_shared_ptr_const_MR_Object_Get()` returning null, but is ultimately orthogonal.
...
MRC_API void MR_std_shared_ptr_const_MR_Object_Construct(const MR_Object *ptr);
