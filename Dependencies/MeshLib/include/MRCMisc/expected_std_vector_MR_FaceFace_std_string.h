...
/// Stores either a `std::vector<MR::FaceFace>` that represents success or a `std::string` that represents an error.
...
/// The array must be destroyed using `MR_expected_std_vector_MR_FaceFace_std_string_DestroyArray()`.
/// Use `MR_expected_std_vector_MR_FaceFace_std_string_OffsetMutablePtr()` and `MR_expected_std_vector_MR_FaceFace_std_string_OffsetPtr()` to access the array elements.
...
/// Destroys a heap-allocated instance of `MR_expected_std_vector_MR_FaceFace_std_string`. Does nothing if the pointer is null.
...
/// Destroys a heap-allocated array of `MR_expected_std_vector_MR_FaceFace_std_string`. Does nothing if the pointer is null.
...
/// If this instance represents success, returns the stored `std::vector<MR::FaceFace>`. Otherwise null.
/// Parameter `_this` can not be null. It is a single object.
...
/// If this instance represents success, returns the stored `std::vector<MR::FaceFace>`. Otherwise null. This version returns a mutable pointer.
/// Parameter `_this` can not be null. It is a single object.
